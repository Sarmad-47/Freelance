import React, { useState } from "react";

const Hero = () => {
  const [banner, setBanner] = useState("50% off");
  const [bannercolor, setBannercolor] = useState("grey");
  const [bannerTextColor, setBannerTextColor] = useState("#000000");
  const [bgcolor, setBgcolor] = useState("#FFFFFF");
  const [name, setName] = useState("Name");
  const [isEditingName, setIsEditingName] = useState(false);

  const [editingName, setEditingName] = useState(false);
  const [bio1, setBio1] = useState("Biography Text");
  const [bio2, setBio2] = useState("Biography Text");
  const [bio3, setBio3] = useState("Biography Text");
  const [bio4, setBio4] = useState("Biography Text");
  const [newDivColor, setNewDivColor] = useState("#FFFFFF");
  const [subcount, setSubCount] = useState("192000");
  const [payments, setPayments] = useState("2000");
  const handleNameEdit = () => {
    setIsEditingName(true);
  };
  const handleNameSave = () => {
    setIsEditingName(false);
  };
  
  return (
    <div className="main h-screen" style={{ backgroundColor: bgcolor }}>
      <div
        className="banner w-full h-10 m-0 pt-2 text-center relative"
        style={{ backgroundColor: bannercolor }}
      >
        <p className="text-lg font-semibold" style={{ color: bannerTextColor }}>
          {banner}
        </p>
        <input
          type="color"
          className="absolute top-2 right-2 w-6 h-6"
          value={bannercolor}
          onChange={(e) => setBannercolor(e.target.value)}
        />
      </div>
      <div className="flex flex-col items-center">
        <div className="mt-10">
          <img
            className="rounded-full h-32 w-32"
            src="data:image/jpeg;base64,"
            alt=""
          />
        </div>
        <div className="flex flex-col items-center mt-6">
        <div className="flex mb-10 space-x-10">
          <div>
            <p className="text-xl font-semibold">{subcount}</p>
            <p>Subscriber Count</p>
          </div>
          <div>
  {isEditingName ? (
    <input
      type="text"
      value={name}
      onChange={(e) => setName(e.target.value)}
      className="border p-1"
    />
  ) : (
    <p className="inline-block">{name}</p>
  )}
  <button
    className="ml-2 p-1 bg-blue-500 text-white rounded"
    onClick={handleNameEdit}
  >
    🖉
  </button>
  {isEditingName && (
    <button
      className="ml-2 p-1 bg-blue-500 text-white rounded"
      onClick={handleNameSave}
    >
      <span style={{ color: 'white' }}>✔️</span>
    </button>
  )}
</div>
          <div>
            <p className="text-xl font-semibold">{payments}</p>
            <p>Payments Sold</p>
          </div>
        </div>
        
        <div className="mt-20">
          <p className="text-xl font-semibold">{bio1}</p>

          <p className="text-xl font-semibold">{bio2}</p>

          <p className="text-xl font-semibold">{bio3}</p>

          <p className="text-xl font-semibold">{bio4}</p>
        </div>
        </div>
        <input
        type="color"
        className="absolute bottom-4 right-4 w-6 h-6 border rounded cursor-pointer"
        value={bgcolor}
        onChange={(e) => setBgcolor(e.target.value)}
        style={{ boxShadow: "0 0 5px rgba(0,0,0,0.2)" }}
      />
    
      </div>
    </div>
  );
};

export default Hero;
